void analysis(Int_t count = 0, TString rootFilePath = "build/output0.root")
{
    ...
    for (Int_t i = 0; i != nEntries; ++i)
    {
        ...
        if (fElectrons == 1 && fDecay_t2 == 1 && fInFe==1)
        {
            ++nEl;
        }
    }
    ...
    for (Int_t i = 0; i != nEntries; ++i)
    {
        ...
        bool isIron = std::find(std::begin(ID_vector), std::end(ID_vector), fEventID_t3) != std::end(ID_vector);
        if (fElectronsStop == 1 && StopInFe_t3 == 1 && isIron == true )
            ++nElAbs;
    }


    std::cout << "Numbers muons decay " << (count + 1) * 0.5 << " cm of Iron : " << nMu << "\n";
    std::cout << "Numbers of electron that were produced in " << (count + 1) * 0.5 << " cm of Iron : " << nEl << "\n";
    std::cout << "Numbers of electron that were produced in " << (count + 1) * 0.5 << " cm of Iron, absorbed : " << nElAbs << "\n";
    std::cout << "Numbers of electron that were produced in " << (count + 1) * 0.5 << " cm of Iron, not absorbed : " << nEl - nElAbs << "\n";
}